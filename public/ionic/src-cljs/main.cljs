...
(def.controller main.ProjectsCtrl [$scope]
  (! $scope.projects
     #js [#js {:title "Test 1" :id 1}
          #js {:title "Test 2" :id 2}]))


(def.controller main.AppCtrl [$scope $ionicModal $timeout $http $ionicPopup baseUrl]
  (! $scope.loadModals 
     (fn [after]
       (do
     ...
     (fn []
       (let [postData #js{:method  "POST"
         ...
         (doto ($http postData)
           (.success (fn [res] 
                       (do
                         (! $scope.token (.-accessToken res))
                         (.hide $scope.modalLogin)
                         (prn (str "token: " $scope.token)))))
