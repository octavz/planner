...
import org.planner.dao.impl.{SlickOauth2DAO, SlickUserDAO}
...
class UserDaoSpec extends BaseDaoSpec {
  ...
  "User org.planner.dao" should {
    ...
    "insertUser,getUserById, getUserByEmail" in new WithApplication {
      val usr = User(guid, guido, 0, guido, nowo, nowo, nowo, 0, guido)
      ...
      v === usr
      val ret1 = Await.result(dao.getUserByEmail(usr.login.get), Duration.Inf)
      ret1 must beRight
