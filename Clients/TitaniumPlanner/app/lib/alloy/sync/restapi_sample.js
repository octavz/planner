...
// Global URL variable
var BASE_URL = Alloy.CFG.App.ApiUrl + '/project/';
...
module.exports.sync = function(method, model, options) {
	...
	case 'create':
		Ti.API.info(payload);
		if (payload.name) {
			http_request('POST', BASE_URL, {
				name : payload.name
			}, callback);
		} else {
			error = 'ERROR: Cannot create model without a name!';
		}
		break;
	...
	case 'update':
		if (payload[model.idAttribute]) {
			http_request('PUT', BASE_URL + payload[model.idAttribute], {
				name : payload.name
			}, callback);
		} else {
			error = 'ERROR: Model does not have an ID!';
